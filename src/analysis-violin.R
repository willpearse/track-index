...
# Do the plots
.outline.plot <- function(climate, quant, clean=NA, se.thresh=5, abs=FALSE, index="bootstrap.index"){
    # Prep simulated data
    ...
    sim.data$alpha.cat <- ifelse(sim.data$alpha==0.5, "intermediate", ifelse(sim.data$alpha==0, "static", "perfect tracking"))
    sim.data$j.a.cat <- factor(sim.data$alpha.cat, levels=c("static","intermediate","perfect tracking"))
    ...
    eval(substitute(
        with(data, plot(index ~ jitter(j.taxon, factor=1.25), pch=20, col=alpha(cols,.25)[taxon], cex=.75, cex.lab=1.5, axes=FALSE, xlab="", xlim=c(1,10.5), ylab=expression(track[XXX](YYY)))), list(XXX=q.var, YYY=c.var)
    ))    
